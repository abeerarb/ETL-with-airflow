import json
import logging
from dev.units_statistics.globals import Globals
import requests as _ApiCaller
from airflow.decorators import task
import pandas as pd

logger = logging.getLogger(__name__)


class Utils:   
    @staticmethod
    def get_all_sim_cards_api(start,limit,child_token):
        data_payload = {"data": json.dumps({"offset": start, "limit": limit,"projection": ["basic","last_update"]})}
        response = _ApiCaller.post(Globals.api_endpoint+"units/lists?token="+child_token, data_payload) 
        if response.status_code == 200:
            return response.json()['data']
        else:
            logger.error("Error:", response.status_code)     
    @staticmethod
    def get_all_units_api(start,limit,child_token):
        data_payload = {"data": json.dumps({"offset": start, "limit": limit,"projection": ["basic","last_update"]})}
        response = _ApiCaller.post(Globals.api_endpoint+"units/lists?token="+child_token, data_payload) 
        if response.status_code == 200:
            return response.json()['data']
        else:
            logger.error("Error:", response.status_code)

    @staticmethod
    def units_transform(units):
    
        units_update = []
        for unit in units:
                if unit.get('name', '') == '':
                     val= unit.get('_id',' ')
                else:
                     val = unit.get('name', '')
                units_update.append({
                    "Unit":val,
                    "unit_id": str(unit.get('_id','')),
                    "spd": unit['last_update'].get('spd', 0) if 'last_update' in unit else 0,
                    "acc":  unit['last_update'].get('acc', 0) if 'last_update' in unit else 0,
                    "last_update": unit['last_update'].get('dtt', 0) if 'last_update' in unit else 0,
                    "device_imei": str(unit.get('imei', '')),
                    "device_serial": str(unit.get('device_serial', '')),
                    "sim_number": str(unit.get('sim_number', '')),
                    "sim_serial": str(unit.get('sim_serial', '')),
                    "device": str(unit.get('device', '')),
                    "company":unit.get('company',''), 
                    "branch": unit.get('city',''),
                    "user_id_units" :str(unit.get('user_id','')),
                    "account": str(unit.get("owner",'')),
                    "operation_code": unit.get('operation_code',0),
                    "vehicle_battery": unit['last_update']['prms'].get('ePwrV', 0) if 'last_update' in unit and 'prms' in unit['last_update'] else 0
                })
        return units_update
        
  


    